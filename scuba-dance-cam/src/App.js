...
function App() {
  ...
  const [instruction, setInstruction] = useState("TUNGGUIN TANGAN MU MUNCUL DI KAMERA...");
  ...
  return (
    <div className="App" onClick={() => audioRef.current && audioRef.current.load()}>
      <div className="container">
        
        <audio ref={audioRef} src={musicFile} loop style={{display: 'none'}} />

        <div className={`header ${danceActive ? 'detected' : ''}`}>
          {instruction}
        </div>

        <div className="video-container" style={{ position: 'relative', width: VIDEO_WIDTH, height: VIDEO_HEIGHT }}>
          <video ref={videoRef} className="input_video" autoPlay playsInline muted style={{display: 'none'}}></video>
          
          <canvas ref={canvasRef} className="output_canvas" width={`${VIDEO_WIDTH}px`} height={`${VIDEO_HEIGHT}px`}></canvas>
        ...
        <div className="feyy-footer" style={{marginTop: '20px', color: 'gray'}}>
          made from Feyy
        </div>
